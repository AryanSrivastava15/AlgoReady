...
export async function getMyFriends(req, res){
    try {
        const user = await User.findById(req.user.id).select("friends").populate("friends", "fullName profilePic college fieldOfStudy");
...
export async function sendFriendRequest(req, res){
    try {
        ...
        if(existingRequest){
            return res.status(400).jsom({ message: "A friend request already exists between you and this user" });
        }
...
export async function getFriendRequests(req, res){
    try {
        const incomingRequests = await FriendRequest.find({
            ...
            status: "pending",
        }).populate("sender", "fullName profilePic college fieldOfStudy");
...
export async function getOutgoingFriendReqs(res, req) {
    try {
        const outgoingRequests = await FriendRequest.find({
            sender: req.user.id,
            status: "pending",
        }).populate("recipient", "fullName profilePic college fieldOfStudy");
